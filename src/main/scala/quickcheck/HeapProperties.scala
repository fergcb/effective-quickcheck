  ...
  val meldingSmallHeaps: (String, Prop) =
    ...
    forAll { (x: Int, y: Int) =>
      ...
      val meldedHeap: List[Node] =
        val heap1 = insert(min, insert(min, empty))
        val heap2 = insert(max, insert(max, empty))
        meld(heap1, heap2)
      ...
      val deleteTwoMinAndFindMin: Boolean =
        val maxHeap = deleteMin(deleteMin(meldedHeap))
        findMin(maxHeap) == max
      ...
      val insertMinAndFindMin: Boolean =
        val minAndMeldedHeap = insert(min, meldedHeap)
        findMin(minAndMeldedHeap) == min
